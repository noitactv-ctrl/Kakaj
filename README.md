...
and payments. Telegram polling and the VPS interval maintenance jobs are not
run inside Vercel serverless functions; keep the existing VPS process or a
separate always-on worker for those jobs. The VPS setup remains available in
[`HOSTING.md`](./HOSTING.md).